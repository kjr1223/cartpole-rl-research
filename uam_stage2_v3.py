...
SAVE_DIR    = os.path.join(BASE, "uam_checkpoints_stage2_v3")
LOG_FILE    = os.path.join(BASE, "uam_stage2_v3_training.log")
CURVE_PNG   = os.path.join(BASE, "uam_stage2_v3_curves.png")
...
def main():
    ...
    for episode in range(1, EPISODES + 1):
        ...
        if episode % EVAL_EVERY == 0:
            ...
            if eval_r > best_eval:
                best_eval = eval_r
                agent.save(os.path.join(SAVE_DIR, "stage2_v3_best.pth"))
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} ★ 최고 → stage2_v3_best.pth\n")
            else:
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} (최고: {best_eval:.2f})\n")

        if episode % SAVE_EVERY == 0:
            agent.save(os.path.join(SAVE_DIR, f"stage2_v3_ep{episode}.pth"))

    agent.save(os.path.join(SAVE_DIR, "stage2_v3_final.pth"))

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    fig.suptitle("UAM Stage2 v3 — Curves", fontsize=13)
