...
class SwitchingEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.total_steps = 0
        self.prev_energy = self._energy()  # 이전 에너지 저장
    ...
    def step(self, action):
        ...
        if self.mode == "swingup":
            # 목표 에너지: 막대가 완전히 위에 있을 때의 에너지
            # E_goal = m*g*l*2 (위치에너지 최대)
            E_goal = self.mass_pole * self.gravity * self.length * 2
            E_now  = current_energy

            # 에너지 오차: 목표 에너지랑 얼마나 차이나는지
            energy_error = abs(E_goal - E_now)

            # reward 구성:
            # 1. 에너지 오차가 작을수록 좋음 (목표에너지에 가까울수록)
            # 2. 위로 올라갈수록 추가 보상
            # 3. 카트 중앙 유지
            reward = (- 0.5 * energy_error          # 에너지 오차 패널티
                     + np.cos(theta)                 # 위치 보상
            ...
            reward = (np.cos(theta)
                     - 0.1 * abs(x)
                     - 0.1 * theta_dot**2
                     + (3.0 if just_switched else 0.0))  # 전환 성공 보너스
    ...
    def _render(self):
        ...
        if not self.screen:
            ...
            pygame.display.set_caption("에너지 기반 스윙업")
        ...
        E_goal = self.mass_pole*self.gravity*self.length*2
        E_now  = self._energy()
        e_ratio = min(E_now/E_goal, 1.0)
        gx, gy, gw = 80, 40, 440
        pygame.draw.rect(self.screen,(200,200,200),(gx,gy,gw,15))
        pygame.draw.rect(self.screen,(255,165,0),(gx,gy,int(gw*e_ratio),15))

        # 각도 게이지
        gy2 = 70
        pygame.draw.rect(self.screen,(200,200,200),(gx,gy2,gw,15))
        pygame.draw.rect(self.screen,(100,200,100),
                        (gx,gy2,int(gw*(theta+np.pi)/(2*np.pi)),15))
        for angle,color in [(0,(255,0,0)),
                            (self.switch_angle,(255,165,0)),
                            (-self.switch_angle,(255,165,0))]:
            sx = gx+int(gw*(angle+np.pi)/(2*np.pi))
            pygame.draw.line(self.screen,color,(sx,gy2-5),(sx,gy2+20),2)
        ...
        self.screen.blit(font2.render(
            f"에너지:{E_now:.2f}/{E_goal:.2f} ({e_ratio*100:.0f}%)",
            True,(150,100,0)),(gx,gy-18))
        self.screen.blit(font2.render(
...
class SACActor(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(128,128), nn.ReLU())  # 신경망 크기 키움
...
# SAC 학습
# ================================
def train_sac(episodes=2000, use_marking=False):
    env      = SwitchingEnv()
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # 마킹 보너스: swingup 모드 + 전환 기준 근처일 때만
        ...
        if (ep+1)%100==0:
            ...
            print(f"ep={ep+1} | reward={total_r:.1f} | "
                  f"평균={avg_r:.1f} | 전환성공={avg_sw}회/100")
            switch_count=0
...
# 그래프
# ================================
def plot_comparison(base_r,base_sw,mark_r,mark_sw):
    def mavg(data,w=10):
        return [np.mean(data[max(0,i-w):i+1]) for i in range(len(data))]
    fig,axes=plt.subplots(1,2,figsize=(14,5))
    fig.suptitle('Baseline SAC vs Marking SAC', fontsize=14, fontweight='bold')
    eps=range(1,len(base_r)+1)
    axes[0].plot(eps,mavg(base_r),color='steelblue',linewidth=2,label='Baseline SAC')
    axes[0].plot(eps,mavg(mark_r),color='tomato',   linewidth=2,label='Marking SAC')
    axes[0].set_title('Learning Curve'); axes[0].set_xlabel('Episode')
    axes[0].set_ylabel('Reward'); axes[0].legend(); axes[0].grid(True,alpha=0.3)
    w=50
    ...
    axes[1].plot(eps,br,color='steelblue',linewidth=2,label='Baseline SAC')
    axes[1].plot(eps,mr,color='tomato',   linewidth=2,label='Marking SAC')
    axes[1].set_title('Switch Success Rate % (last 50 ep)')
    ...
    print("그래프 저장: energy_comparison.png")
...
if __name__=="__main__":
    ...
    base_actor,base_r,base_sw=train_sac(episodes=2000,use_marking=False)
    mark_actor,mark_r,mark_sw=train_sac(episodes=2000,use_marking=True)
    visualize(base_actor,balance_actor,label="기본 SAC",episodes=3)
    visualize(mark_actor,balance_actor,label="마킹 SAC",episodes=3)
    plot_comparison(base_r,base_sw,mark_r,mark_sw)

    print("\n===== 최종 요약 =====")
    print(f"기본 SAC 전체 평균: {np.mean(base_r):.1f}")
    print(f"마킹 SAC 전체 평균: {np.mean(mark_r):.1f}")
    print(f"기본 SAC 전환 성공: {sum(base_sw)}회 / 2000 에피소드")
    print(f"마킹 SAC 전환 성공: {sum(mark_sw)}회 / 2000 에피소드")
