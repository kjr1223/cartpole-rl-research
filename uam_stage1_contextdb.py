"""
uam_stage1_contextdb.py
=======================
Stage 1 (20~40m) — SAC + Context DB + Ontology
제안 방법의 핵심 컴포넌트를 Stage 1 고도에서 적용
...
SAVE_DIR    = os.path.join(BASE, "uam_checkpoints_stage1_contextdb")
LOG_FILE    = os.path.join(BASE, "uam_stage1_contextdb.log")
CURVE_PNG   = os.path.join(BASE, "uam_stage1_contextdb_curves.png")
...
def main():
    ...
    log(f"\n{'='*60}")
    log(f"Stage 1 — SAC + Context DB + Ontology")
    log(f"  고도: {ALT_LOW}~{ALT_HIGH}m | max_steps: {MAX_STEPS} ({MAX_STEPS*0.0025:.1f}초)")
    log(f"  에피소드: {EPISODES} | alpha: {FIXED_ALPHA}")
    ...
    for episode in range(1, EPISODES + 1):
        ...
        if episode % EVAL_EVERY == 0:
            ...
            if eval_r > best_eval:
                best_eval = eval_r
                agent.save(os.path.join(SAVE_DIR, "stage1_contextdb_best.pth"))
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} ★ → stage1_contextdb_best.pth\n")
            else:
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} (최고: {best_eval:.2f})\n")

        if episode % SAVE_EVERY == 0:
            agent.save(os.path.join(SAVE_DIR, f"stage1_contextdb_ep{episode}.pth"))

    agent.save(os.path.join(SAVE_DIR, "stage1_contextdb_final.pth"))

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    fig.suptitle("Stage 1 — SAC + Context DB + Ontology", fontsize=12)
