...
class CartPoleSwingUpContinuous(gym.Env):
    def __init__(self, render_mode=None):
        ...
        self.current_action = 0.0  # 현재 행동값 시각화용
    ...
    def _render(self, agent_name="Agent"):
        ...
        action_ratio = (self.current_action + 1.0) / 2.0  # -1~+1 → 0~1
        action_color = (255,100,0) if self.current_action > 0 else (0,100,255)
        mid_x = gauge_x + gauge_w // 2
        bar_w = int(abs(self.current_action) * gauge_w / 2)
        if self.current_action > 0:
            pygame.draw.rect(self.screen, action_color,
        ...
        self.screen.blit(
            ...
            (gauge_x, gauge_y - 18))
        self.screen.blit(
            font2.render(f"행동값: {self.current_action:.3f} (왼쪽←  →오른쪽)",
                        True, (100,100,100)),
...
def visualize(model, agent_name, episodes=3):
    ...
    env.screen  # 화면 초기화 트리거

    print(f"\n{agent_name} 시각화 시작!")
    print("창 닫으면 다음 에이전트로 넘어가요")
    ...
    for ep in range(episodes):
        ...
        print(f"  에피소드 {ep+1}: {step}스텝 | reward={total_reward:.1f}")
...
print("PPO 모델 시각화")
print("=" * 40)
ppo_model = ContinuousPolicyNetwork()
...
print("SAC 모델 시각화")
print("=" * 40)
sac_model = SACActorContinuous()

# SAC actor만 따로 저장이 안 돼있어서 다시 학습 후 저장
print("SAC 모델 빠르게 재학습 중... (잠깐만요)")
...
# 간단히 200 에피소드만 재학습
env_train  = CartPoleSwingUpContinuous()
...
for episode in range(200):
    ...
    while not done:
        ...
        action_val = action.squeeze().item()
        next_obs, reward, terminated, truncated, _ = env_train.step(action_val)
        done = terminated or truncated
        ...
        if len(buffer) >= 64:
            ...
            new_a, new_lp = actor.get_action(s)
    ...
    if (episode+1) % 50 == 0:
        print(f"  SAC 재학습 {episode+1}/200 완료")

print("SAC 재학습 완료!")
visualize(actor, "SAC", episodes=3)
print("\n시각화 완료!")
