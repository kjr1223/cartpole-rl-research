...
uam_stage1_land.py
==================
Stage 1 (20~40m) + max_steps 6,000 착지 달성 학습

[목표]
  - 30m ÷ (2 m/s × 0.0025 s/step) = 6,000 스텝 → 물리적으로 착지 가능
  - Stage 2에서 VRS 회피 + 하강 학습 완료 → Stage 1에서 착지 정밀 제어 학습

[설정]
  - 시작점: stage2_best.pth (Eval +3,005)
  - 고도: 20~40m, max_steps=6,000
  - alpha=0.03, buffer=100k, episodes=1,000
  - 보상 구조: shaping(+0.05×alt_drop) + 착지 +500 유지
  - 체크포인트: uam_checkpoints_stage1/
  - 로그: uam_stage1_training.log
"""
...
CKPT_LOAD   = os.path.join(BASE, "uam_checkpoints_stage2", "stage2_best.pth")
SAVE_DIR    = os.path.join(BASE, "uam_checkpoints_stage1")
LOG_FILE    = os.path.join(BASE, "uam_stage1_training.log")
CURVE_PNG   = os.path.join(BASE, "uam_stage1_curves.png")

ALT_LOW     = 20.0
ALT_HIGH    = 40.0
MAX_STEPS   = 6_000    # 30m ÷ (2 m/s × 0.0025 s) = 6,000 스텝
EPISODES    = 1_000
BUFFER_SIZE = 100_000
FIXED_ALPHA = 0.03
EVAL_EVERY  = 100
...
def main():
    ...
    log(f"\n{'='*55}")
    log(f"Stage 1 착지 달성 학습")
    log(f"  고도: {ALT_LOW}~{ALT_HIGH}m | max_steps: {MAX_STEPS} ({MAX_STEPS*0.0025:.1f}초)")
    log(f"  생존보상 최대: +{MAX_STEPS} | 착지보너스: +500 (비중 {500/MAX_STEPS*100:.1f}%)")
    ...
    for episode in range(1, EPISODES + 1):
        ...
        if episode % EVAL_EVERY == 0:
            ...
            if eval_r > best_eval:
                best_eval = eval_r
                agent.save(os.path.join(SAVE_DIR, "stage1_best.pth"))
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} ★ 최고 갱신 → stage1_best.pth\n")
            else:
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} (최고: {best_eval:.2f})\n")

        if episode % SAVE_EVERY == 0:
            agent.save(os.path.join(SAVE_DIR, f"stage1_ep{episode}.pth"))

    agent.save(os.path.join(SAVE_DIR, "stage1_final.pth"))

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    fig.suptitle("UAM Stage1 Landing — Curves", fontsize=13)
    ep_arr = np.array(ep_rewards)
    x_ep   = np.arange(1, len(ep_rewards) + 1)
    w = 20
    ma = np.convolve(ep_arr, np.ones(w)/w, mode='valid')
    axes[0].plot(x_ep, ep_arr, alpha=0.3, color='#D69E2E', linewidth=0.7)
    axes[0].plot(x_ep[w-1:], ma, color='#D69E2E', linewidth=1.8, label=f'MA({w})')
