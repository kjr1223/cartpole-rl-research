"""
uam_eval_vrs_rate.py
====================
학습된 Stage 1 모델의 VRS 진입률 측정 (평가만, 학습 없음)
100 에피소드 돌려서 VRS 진입률 계산
"""
...
CKPT     = os.path.join(BASE, "uam_checkpoints_fresh", "fresh_best.pth")
N_EVAL   = 200   # 평가 에피소드 수
...
def main():
    ...
    total_vrs_steps = 0
    landing_count   = 0
    vrs_episode_count = 0  # VRS에 한 번이라도 진입한 에피소드 수
    ...
    print(f"\n{'='*55}")
    print(f"[제안 방법 평가 완료] Stage 1, {N_EVAL} 에피소드")
