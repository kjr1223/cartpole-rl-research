...
cursor = conn.cursor()
cursor.execute("""
    CREATE TABLE IF NOT EXISTS switching_markings (
...
""")
conn.commit()
print("DB 준비 완료: switching_markings 테이블")
...
if __name__ == "__main__":
    ...
    print("\n=== 전환 시점 마킹 ===")
    print("← → : 카트 조종 (스윙업)")
    print("SPACE: 전환되는 순간 마킹! (이 state가 전환 시점!)")
    print("R: 리셋 | Q: 종료")
    print("====================\n")
    ...
    while running:
        ...
        if space_pressed:
            ...
            cursor.execute("""
                INSERT INTO switching_markings
    ...
    print("저장 위치: marking_data.db / switching_markings 테이블")
