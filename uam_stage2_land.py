...
uam_stage2_land.py
==================
Stage 2 (60~90m) + max_steps 6,000 착지 달성 학습

[목표]
  - max_steps=12,000에서 생존 보상(최대 +12,000) >> 착지 보너스(+500) 문제 해결
  - max_steps=6,000 → 생존 보상 최대 +6,000, 착지 보너스 비중 +8%로 향상
  - 고도 60~90m (Stage 2): 물리적으로 착지 가능한 범위
    → 평균 75m / 15초 = 5 m/s 하강 → VRS SAFE 범위

[설정]
  - 시작점: stage3_ep1100.pth (VRS 회피 이미 학습됨)
  - 고도: 60~90m, max_steps=6,000
  - alpha=0.03, buffer=100k, episodes=1,000
  - 보상 구조: shaping(+0.05×alt_drop) + 착지 +500 유지
  - 체크포인트: uam_checkpoints_stage2/
  - 로그: uam_stage2_training.log
"""
...
SAVE_DIR    = os.path.join(BASE, "uam_checkpoints_stage2")
LOG_FILE    = os.path.join(BASE, "uam_stage2_training.log")
CURVE_PNG   = os.path.join(BASE, "uam_stage2_curves.png")

ALT_LOW     = 60.0
ALT_HIGH    = 90.0
MAX_STEPS   = 6_000    # 6,000 × 0.0025s = 15초, 평균 5 m/s 하강 가능
EPISODES    = 1_000
BUFFER_SIZE = 100_000
FIXED_ALPHA = 0.03
EVAL_EVERY  = 100
...
def main():
    ...
    for episode in range(1, EPISODES + 1):
        ...
        if episode % EVAL_EVERY == 0:
            ...
            if eval_r > best_eval:
                best_eval = eval_r
                agent.save(os.path.join(SAVE_DIR, "stage2_best.pth"))
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} ★ 최고 갱신 → stage2_best.pth\n")
            else:
                log(f"\n[Eval] Episode {episode} | Eval: {eval_r:.2f} (최고: {best_eval:.2f})\n")

        if episode % SAVE_EVERY == 0:
            agent.save(os.path.join(SAVE_DIR, f"stage2_ep{episode}.pth"))

    agent.save(os.path.join(SAVE_DIR, "stage2_final.pth"))

    # 학습 곡선
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    fig.suptitle("UAM Stage2 Landing — Curves", fontsize=13)
